...
type UIAction =
  ...
  | { type: 'SET_SHOP_CATEGORY'; payload: 'all' | 'themes' | 'skins' | 'powerups' | 'coins' }
...
export const useUIState = () => {
  ...
  const setShopCategory    = useCallback((v: 'all' | 'themes' | 'skins' | 'powerups' | 'coins') => dispatch({ type: 'SET_SHOP_CATEGORY', payload: v }), []);
